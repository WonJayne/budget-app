...
Rules are case-insensitive description substring matches. Each rule has a visible **Inflow**, **Outflow**, or **Internal transfer** type. Inflow rules apply only to positive transactions, outflow rules apply only to negative transactions, and internal-transfer rules can apply to either sign because bank exports may show either side of an internal movement. Internal-transfer rules also have an optional transfer sign scope: `any`, `transfer in only`, or `transfer out only`. Rules can be global with Source = `Any`, or source-scoped to one stable import source such as `ZKB private account`, `Viseca credit card`, or `Manual`. Priority is respected, and source-specific rules win over global rules with the same priority. Manual assignments survive rule reapplication. Editing or deleting rules removes stale rule-based classifications, including stale rule-based transfers.
...
Internal transfer direction is a monitoring concept, not a budget inflow/outflow. For transactions with `cash_flow_type="transfer"`, the app derives `transfer_direction` from the amount: positive amounts are shown as **Internal transfer in**, negative amounts as **Internal transfer out**, and zero amounts have no transfer direction. Ordinary budget movements continue to be shown as **Inflow** and **Outflow**.
