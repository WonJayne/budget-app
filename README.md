...
- adding manual inflow, outflow, internal-transfer-in, and internal-transfer-out entries in the selected All/Year/Month period;
...
Rules are case-insensitive description substring matches. Each rule has a visible applicability choice: **Inflow**, **Outflow**, **Any internal transfer**, **Internal transfer in**, or **Internal transfer out**. Inflow rules apply only to positive ordinary transactions, outflow rules apply only to negative ordinary transactions, and internal-transfer rules can apply to any transfer sign or only one transfer direction because bank exports may show either side of an internal movement. Rules can be global with Source = `Any`, or source-scoped to one stable import source such as `ZKB private account`, `Viseca credit card`, or `Manual`. Priority is respected, and source-specific rules win over global rules with the same priority. Manual assignments survive rule reapplication. Editing or deleting rules removes stale rule-based classifications, including stale rule-based transfers.
...
Imported CSV entries can be edited in the ledger when a bank export needs correction. Editing preserves the original transaction ID so duplicate detection and saved state stay consistent. The edit dialog lets users change date, flow type, description, amount, currency, account, category, owner, optional transfer matching fields, and ignored status. Flow type is chosen as **Inflow**, **Outflow**, **Internal transfer in**, or **Internal transfer out**. The amount field is treated as an absolute value: the selected flow type normalises storage so inflows and internal-transfer-ins are positive, while outflows and internal-transfer-outs are negative.
...
Manual entries are stored as local transactions with `source_kind="manual"`/`entry_source="manual"`, `import_source="Manual"`, no source file, and manual assignment provenance. Manual dialogs use the same four flow choices (**Inflow**, **Outflow**, **Internal transfer in**, **Internal transfer out**) and treat the amount input as an absolute amount. The selected flow type determines the stored sign. Optional transfer matching fields such as transfer group and note are available in an advanced section. Manual entries can be added from the existing manual-entry controls and edited later through the same ledger edit dialog.
...
Internal transfer direction is a monitoring concept, not a budget inflow/outflow. In the UI, users choose **Internal transfer in** or **Internal transfer out** directly instead of typing a signed transfer amount. In storage, transactions still use `cash_flow_type="transfer"`, and `transfer_direction` is derived from the stored amount sign: positive amounts are shown as **Internal transfer in**, negative amounts as **Internal transfer out**. Ordinary budget movements continue to be shown as **Inflow** and **Outflow**.
...
- **Export full backup** writes a local JSON file containing complete app state: schema metadata, profile options, transactions, manual entries, import sources, cash-flow/transfer classifications, derived transfer directions, transfer group IDs and notes, edited ledger fields, ignored flags, assignment provenance, source-scoped rules, transfer sign scopes, and category colours.
- **Import full backup** replaces the complete current app state with the JSON backup.
- **Export rules/profile** writes JSON containing schema metadata, derived/default selector options, source-scoped inflow/outflow/internal-transfer rules with transfer sign scopes, and category colours, with no transaction ledger data.
