...
- Rules can be global with Source = **Any** or source-scoped to an observed import source. Inflow rules classify positive transactions only; outflow rules classify negative transactions only; internal-transfer rules can classify either sign and can optionally target transfer in only or transfer out only. Manual assignments and transfer groups/notes survive rule changes.
