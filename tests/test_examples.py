...
def test_sample_transactions_cover_multiple_months_accounts_and_categories() -> None:
    ...
    assert {transaction.account for transaction in transactions} == {"shared_zkb", "flo_card", "wife_card"}
    assert sum(1 for transaction in transactions if transaction.amount > 0) == 4
...
def test_sample_state_loads_with_rich_rules_colours_and_classified_transactions() -> None:
    ...
    assert any(transaction.owner == "wife" for transaction in state.transactions)
